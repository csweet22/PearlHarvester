...
public class Navigator3D : MonoBehaviour
{
    ...
    private void RecalculatePath()
    {
        ...
        foreach (KeyValuePair<Vector3Int, Sample> kvp in _grid){
            kvp.Value.DistanceToTarget = Vector3.Distance(kvp.Value.WorldPosition, target.position);
            if (kvp.Value.DistanceToTarget < closestDistance){
                closestDistance = kvp.Value.DistanceToTarget;
    ...
    private void OnDrawGizmos()
    {
        foreach (KeyValuePair<Vector3Int, Sample> kvp in _grid){
            ...
            Gizmos.color =  Color.Lerp(Color.red, Color.green, sample.DistanceToTarget / (scale * resolution / 2));
